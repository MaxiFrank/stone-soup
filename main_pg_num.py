...
def load_contents():
    with open('contents.json') as contents_file:
...
def load_indices():
    with open('indices.json') as indices_file:
                    ...
                    if prev_end_page is not None:
                        if start == prev_end_page:
                            if "story" in values and values["story"]:
                                current_story_score = calculate_score(flatten(values["story"]), individual_words)                    
                                if current_story_score > prev_story_score:
                                    # add tag to current story
                                    contents[id]["tags"] = contents[id].get("tags", [])
                                    contents[id]["tags"].append([num, tag])
                                    # take tag out of previous story
                                    contents[prev_id]["tags"].remove([num,tag])
                    else:
                        if "story" in values and values["story"]:
                            contents[id]["tags"] = contents[id].get("tags", [])
                            contents[id]["tags"].append([num, tag])
                            prev_story_score = calculate_score(flatten(values["story"]), individual_words)
                            prev_id = id
                            prev_end_page = end
